      ...
      try {
        ...
        contents.debugger.on("message", async (evt, method, params) => {
          if (method === "Page.fileChooserOpened" && pendingResumeUpload) {
            ...
            try {
              if (alive()) contents.debugger.detach();
            } catch (e) {}
          }
        });
      } catch (e) {
        ...
        if (
          !e.message?.includes("Render frame") &&
...
app.whenReady().then(() => {
  ...
  app.on("web-contents-created", (_, wc) => {
    wc.on("did-finish-load", () => {
      try {
        wc.executeJavaScript(uaScript).catch(() => {});
      } catch (_) {}
    });
  ...
  console.error = (...args) => {
    const msg = args.join(" ");
    if (
      msg.includes("CertVerifyProcBuiltin") ||
...
ipcMain.handle("shortcuts:save", (_, shortcuts) => {
...
ipcMain.handle(
  ...
  async (
    _,
    { questions, jobContext, jobDescription, userProfile, pageUrl },
  ) => {
    ...
    try {
      ...
      if (
        ...
        return questions.map((q) => ({
          ...
          answer: "",
          error:
            "⚠️ AI Server not running! Start it with: npx nodemon server.js",
        }));
      }
...
ipcMain.handle(
  "engine:smartUploadFile",
  async (_, { webContentsId, selector, filePath }) => {
    try {
...
ipcMain.handle(
  ...
  async (_, { webContentsId, selector, filePath }) => {
    try {
      ...
      if (!nodeId) {
        try {
          const allResult = await wc.debugger.sendCommand(
      ...
      if (!nodeId) {
        ...
        return {
          success: false,
          error: "File input not found with any selector",
        };
      }
...
ipcMain.handle(
  "engine:googleFormsUpload",
  async (_, { webContentsId, filePath }) => {
    try {
      if (!filePath || !fs.existsSync(filePath)) {
        return { success: false, error: "File not found: " + filePath };
      }
      ...
      const uploadPromise = new Promise((resolve, reject) => {
        const timeout = setTimeout(() => {
          ...
          reject(
            new Error(
              "File upload timeout (20s) — try setting resume in standard form",
            ),
          );
        }, 20000);
        ...
        wc.debugger.on("message", async (evt, method, params) => {
          if (method === "Page.fileChooserOpened") {
            console.log(
              "📎 File chooser opened on webview, providing:",
              filePath,
            );
            clearTimeout(timeout);
            pendingResumeUpload = null;
...
ipcMain.handle("app:getSiteData", async () => {
  try {
    ...
    console.error("Error getting site data:", e);
    return [];
  }
});
