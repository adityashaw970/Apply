...
app.post('/api/answer', async (req, res) => {
  ...
  if (!questions || questions.length === 0) {
    return res.status(400).json({ error: 'No questions provided', answers: [] });
  }
  ...
  try {
    ...
    console.log(`📋 Job: ${jobContext?.title || 'Unknown'} at ${jobContext?.company || 'Unknown'}`);
    ...
    console.log(`✅ AI answered ${answers.filter(a => a.answer).length}/${questions.length} questions`);
    ...
    res.status(500).json({
      error: error.message,
      answers: questions.map(q => ({ label: q.label, answer: '', error: error.message }))
    });
  }
});
...
io.on('connection', (socket) => {
  ...
  socket.on('send_message', async (data) => {
    ...
    if (!apiKeys) {
      socket.emit('bot_reply', '❌ Error: No API key configured. Set it in Settings.');
      return;
    }
